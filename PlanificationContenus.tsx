...
import { BookOpen, Calendar, TrendingUp, CheckCircle2, Clock, Download, FileSpreadsheet, BarChart3 } from 'lucide-react';
...
/**
 * COMPOSANT : PLANIFICATION DE CONTENUS PÉDAGOGIQUES
 * Gestion des programmes de la 6ème à la Terminale
 * Intégré au style du panneau admin PedaClic
 */
...
// Vue : Planification
const VuePlanification: React.FC<{
  niveauActif: string;
  setNiveauActif: (n: string) => void;
  trimestreActif: string;
  setTrimestreActif: (t: string) => void;
  disciplineActive: string;
  setDisciplineActive: (d: string) => void;
  contenuActif: ContenuData;
  updateContenu: (n: string, t: string, d: string, c: keyof ContenuData, v: any) => void;
}> = ({ niveauActif, setNiveauActif, trimestreActif, setTrimestreActif, disciplineActive, setDisciplineActive, contenuActif, updateContenu }) => (
  <div>
    {/* Sélecteurs */}
...
const ChampTexte: React.FC<{
  label: string;
  placeholder: string;
  value: string;
  onChange: (val: string) => void;
  rows?: number;
}> = ({ label, placeholder, value, onChange, rows = 1 }) => (
  <div>
    <label style={{ display: 'block', marginBottom: '8px', fontSize: '14px', fontWeight: '600', color: '#2c3e50' }}>
      {label}
    </label>
...
// Vue : Tableau de bord
const VueTableauDeBord: React.FC<{
  prepareDataNiveaux: () => any[];
  prepareDataDisciplines: () => any[];
  prepareDataTrimestres: () => any[];
}> = ({ prepareDataNiveaux, prepareDataDisciplines, prepareDataTrimestres }) => {
  ...
  return (
    <div>
      <h3 style={{ margin: '0 0 25px 0', fontSize: '20px', fontWeight: 'bold', color: '#2c3e50' }}>
      ...
      <div style={{ display: 'grid', gap: '25px' }}>
        ...
        <div style={{ background: '#f8f9fa', padding: '25px', borderRadius: '10px', border: '2px solid #e0e0e0' }}>
          <h4 style={{ margin: '0 0 20px 0', fontSize: '16px', fontWeight: '600', color: '#2c3e50' }}>
